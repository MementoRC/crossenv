...
def test_explicit_manylinux(tmp_path, host_python, build_python, architecture):
    ...
    crossenv = make_crossenv(
        tmp_path, host_python, build_python, "--manylinux=manylinux2014"
    )
...
def test_run_sysconfig_module(crossenv):
    ...
    # sysconfig'
    out = crossenv.check_output(["python", "-m", "sysconfig"], universal_newlines=True)
...
def test_machine_override(tmp_path, host_python, build_python):
    crossenv = make_crossenv(tmp_path, host_python, build_python, "--machine=foobar")
